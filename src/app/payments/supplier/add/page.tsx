  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [loadedTransactions, loadedSuppliers] = await Promise.all([
        fetchBusinessTransactions(userId, undefined, undefined, selectedDateRange === 'all_time'), 
  ...
  const supplierPayments = useMemo(() => {
    ...
    if (selectedDateRange !== "all_time") {
        ...
        if (selectedDateRange === "this_month") {
            startDateRange = startOfMonth(now);
            endDateRange = endOfMonth(now);
        } else if (selectedDateRange === "last_month") {
            const lastMonthDate = subMonths(now, 1);
            startDateRange = startOfMonth(lastMonthDate);
            endDateRange = endOfMonth(lastMonthDate);
        } else if (selectedDateRange === "this_year") {
            startDateRange = startOfYear(now);
            endDateRange = endOfYear(now);
        }
        if(startDateRange && endDateRange) {
            payments = payments.filter(p => {
                const paymentDate = new Date(p.date);
                return paymentDate >= startDateRange! && paymentDate <= endDateRange!;
            });
        }
    }
